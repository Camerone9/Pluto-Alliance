...
//                 await admin.firestore().collection('metaMaskUsers').doc(createdUser.uid).set({
...
//           const userDocRef = admin.firestore().collection('metaMaskUsers').doc(address);
//           console.log(userDocRef, address);
...
//               // Return the token
//               return response.status(200).json({ token: 'Trent Logged In' });
//             } else {
